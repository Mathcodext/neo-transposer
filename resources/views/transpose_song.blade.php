    ...
    @if ($song->song->url)
        <h3 class="song-url">
            <a href="{{ $song->song->url }}" target="_blank">
                @if (str_contains($song->song->url, 'chantscnc') || str_contains($song->song->url, 'ressucite') || app()->getLocale() === 'fr')
                    <img src="{{ url('/static/img/site-ressucite-fr.png') }}" alt="Il est ressuscité" class="site-icon" />
                @endif
